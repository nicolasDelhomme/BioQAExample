...
#' ```
#' This is not needed as the spruce annotation has a single transcript per gene
...
#' ````
stopifnot(all(match(sub("_S[0-9]+_L001_subset_sortmerna.*","",basename(dirname(filelist))),
...
ggplot(dat,aes(x,y,fill=Stage)) + geom_col() + 
...
write.csv(counts,file=here("analysis/salmon/raw-unormalised-gene-expression_data.csv"))
...
save(dds,file=here("analysis/salmon/dds.rda"))
...
#' ```
nlevel=nlevels(dds$Tissue) * nlevels(dds$Stage)
...
p <- ggplot(pc.dat,aes(x=PC1,y=PC2,col=Tissue,shape=Stage,text=SampleID)) + 
  geom_point(size=2) + 
